...
def run(ctx: protocol_api.ProtocolContext):
    ...
    STEPS = { #Dictionary with STEP activation, description, and times
            ...
            4:{'Execute': True, 'description': 'Incubate wait with magnet ON', 'wait_time': 300}, #900
            ...
            21:{'Execute': True, 'description': 'Wait with magnet OFF', 'wait_time': 600},#600
    ...
    VHB = Reagent(name = 'VHB',
                    ...
                    rinse = True,
                    reagent_reservoir_volume = (NUM_SAMPLES + 5) * 400, #60000, #38400
                    num_wells = math.ceil((NUM_SAMPLES + 5) * 400 / 13000), #num_Wells max is 4
                    h_cono = 1.95,
                    v_fondo = 1.95*multi_well_rack_area/2, #Prismatic
                    tip_recycling = 'A1')
    ...
    Water = Reagent(name = 'Water',
                    ...
                    num_wells = math.ceil((NUM_SAMPLES + 5) * 50 / 13000), #num_Wells max is 1
                    h_cono = 1.95,
                    v_fondo = 1.95*multi_well_rack_area/2) #Prismatic
    ...
    pickup_height, rinse, wait_time, blow_out, disposal_volume):
        ...
        if air_gap_vol_bottom != 0: #If there is air_gap_vol, switch pipette to slow speed
            pipet.move_to(source.top(0))
            pipet.air_gap(air_gap_vol_bottom) #air gap
...
###############################################################################
    ...
    #Transfer lysis
        ...
        ctx.comment('###############################################')
        lysis_volume = 140
        lysis_transfer_vol=[lysis_volume + disposal_volume, lysis_volume + disposal_volume, lysis_volume + disposal_volume] # Three rounds of 140 + disposal volume
        x_offset = 0
        rinse = True
        for i in range(num_cols):
            ...
            for j,transfer_vol in enumerate(lysis_transfer_vol):
                ...
                pickup_height = pickup_height, rinse = rinse, wait_time = 0, blow_out = False, disposal_volume = disposal_volume)
                m300.move_to(work_destinations[i].top(0))
                m300.air_gap(air_gap_vol_bottom) #air gap
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        supernatant_volume = 155
        supernatant_vol = [supernatant_volume + disposal_volume, supernatant_volume + disposal_volume, supernatant_volume + disposal_volume, supernatant_volume + disposal_volume]
        x_offset_rs = 2
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_vol:
                ...
                dest = waste, vol = transfer_vol, air_gap_vol_bottom = air_gap_vol_bottom, air_gap_vol_top = 0, x_offset = x_offset,
                pickup_height = pickup_height, rinse = False, wait_time = 0, blow_out = False, disposal_volume = disposal_volume)
                m300.air_gap(air_gap_vol_bottom) #air gap
            if recycle_tip:
                m300.return_tip()
            else:
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        ctx.comment(' ')
        vhb_volume = 133.33
        vhb_wash_vol = [vhb_volume + disposal_volume, vhb_volume + disposal_volume, vhb_volume + disposal_volume]
        x_offset = 0
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_vol:
                ...
                move_vol_multi(m300, reagent = Elution, source = work_destinations[i],
                dest = waste, vol = transfer_vol, air_gap_vol_bottom = air_gap_vol_bottom, air_gap_vol_top = 0, x_offset = x_offset,
                pickup_height = pickup_height, rinse = False, wait_time = 3, blow_out = False, disposal_volume = disposal_volume)
                m300.air_gap(air_gap_vol_bottom) #air gap
            if recycle_tip:
                m300.return_tip()
            else:
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        ctx.comment(' ')
        spr_volume = 166.66
        spr_wash_vol = [spr_volume + disposal_volume, spr_volume + disposal_volume, spr_volume + disposal_volume]
        x_offset = 0
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        ctx.comment('###############################################')
        #Water elution
        water_wash_vol = [50 + disposal_volume]
        x_offset_rs = 2
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in elution_vol:
                ...
                ctx.comment('Pickup height is ' + str(pickup_height) +' (fixed)')
                move_vol_multi(m300, reagent = Elution, source = work_destinations[i],
                dest = final_destinations[i], vol = transfer_vol, air_gap_vol_bottom = air_gap_vol_bottom, air_gap_vol_top = 0, x_offset = x_offset,
                pickup_height = pickup_height, rinse = False, wait_time = 0, blow_out = False, disposal_volume = disposal_volume)
