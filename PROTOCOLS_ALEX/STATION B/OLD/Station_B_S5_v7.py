...
def run(ctx: protocol_api.ProtocolContext):
    ...
    STEPS = { #Dictionary with STEP activation, description, and times
            ...
            24:{'Execute': False, 'description': 'SUPERNATANT - TEST'},
            }
    ...
    class Reagent:
        ...
        air_gap_vol_bottom, air_gap_vol_top, disposal_volume, rinse, reagent_volume, reagent_reservoir_volume, num_wells, h_cono, v_fondo, tip_recycling = 'none'):
            self.name = name
    ...
    Lysis = Reagent(name = 'Lysis',
                    ...
                    flow_rate_aspirate_mix = 3, # Original = 1.5
                    flow_rate_dispense_mix = 5,
                    air_gap_vol_bottom = 5,
                    air_gap_vol_top = 0,
                    disposal_volume = 1,
                    rinse = True,
                    reagent_volume = 420, # reagent volume needed per sample
                    reagent_reservoir_volume =  (NUM_SAMPLES + 5) * 420, #70000, #51648
                    num_wells = math.ceil((NUM_SAMPLES + 5) * 420 / 13000), #num_Wells max is 4, 13000 is the reservoir max volume (eventhough reservoir allows 15000)
                    h_cono = 1.95,
...
###############################################################################
    ...
    #Transfer lysis
        ...
        lysis_trips = math.ceil(Lysis.reagent_volume / max_volume_allowed)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        supernatant_trips = math.ceil((Lysis.reagent_volume + sample_volume) / max_volume_allowed)
        supernatant_volume = max_volume_allowed # We try to remove an exceeding amount of supernatant to make sure it is empty
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        ctx.comment(' ')
        max_volume_allowed = 190 # Tips allow up to 200uL, but we only allow max_volume_allowed
        vhb_trips = math.ceil(VHB.reagent_volume / max_volume_allowed)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        # remove supernatant -> height calculation can be omitted and referred to bottom!
        max_volume_allowed = 190 # Tips allow up to 200uL, but we only allow max_volume_allowed
        supernatant_trips = math.ceil(VHB.reagent_volume / max_volume_allowed)
        supernatant_volume = max_volume_allowed # We try to remove an exceeding amount of supernatant to make sure it is empty
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        ctx.comment(' ')
        max_volume_allowed = 190 # Tips allow up to 200uL, but we only allow max_volume_allowed
        spr_trips = math.ceil(SPR.reagent_volume / max_volume_allowed)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        # remove supernatant -> height calculation can be omitted and referred to bottom!
        max_volume_allowed = 190 # Tips allow up to 200uL, but we only allow max_volume_allowed
        supernatant_trips = math.ceil(SPR.reagent_volume / max_volume_allowed)
        supernatant_volume = max_volume_allowed # We try to remove an exceeding amount of supernatant to make sure it is empty
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        #Water elution
        max_volume_allowed = 190 # Tips allow up to 200uL, but we only allow max_volume_allowed
        water_trips = math.ceil(Water.reagent_volume / max_volume_allowed)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        elution_trips = math.ceil(Elution.reagent_volume / max_volume_allowed)
    ...
    ###############################################################################
    # STEP 24 REMOVE SUPERNATANT - TEST
    ########
    STEP += 1
    if STEPS[STEP]['Execute']==True:
        start = datetime.now()
        ...
        ctx.comment(' ')
        # remove supernatant -> height calculation can be omitted and referred to bottom!
        supernatant_volume = 10
        supernatant_vol = [supernatant_volume + 1]
        x_offset_rs = 2
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_vol:
                ...
                ctx.comment('Pickup height is ' + str(pickup_height) +' (fixed)')
                ctx.delay(seconds=3)
                '''m300.aspirate(location = work_destinations[i], volume = transfer_vol, rate = 0.2)
                m300.move_to(work_destinations[i].top(10))
                ctx.delay(seconds=10)
                m300.move_to(work_destinations[i].top(-10))
                m300.dispense(location = waste, volume = transfer_vol, rate = 0.5)'''
                move_vol_multi(m300, reagent = Elution, source = work_destinations[i],
                dest = waste, vol = transfer_vol, x_offset = x_offset, pickup_height = pickup_height,
                rinse = False, wait_time = 2, blow_out = False)
