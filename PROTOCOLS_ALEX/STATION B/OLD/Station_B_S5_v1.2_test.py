...
def run(ctx: protocol_api.ProtocolContext):
    ...
    Lysis = Reagent(name = 'Lysis',
                    ...
                    rinse = True,
                    reagent_reservoir_volume =  NUM_SAMPLES * 548, #70000, #51648
                    num_wells = 3, #num_Wells max is 4
                    h_cono = 1.95,
    ...
    pickup_height, rinse, wait_time):
        ...
        if air_gap_vol_top !=0: #If there is air_gap_vol, switch pipette to slow speed
            pipet.aspirate(air_gap_vol_top, source.top(z = 2), rate = reagent.flow_rate_aspirate) #air gap
        pipet.aspirate(vol, s) # aspirate liquid
        ...
        # GO TO DESTINATION
        pipet.dispense(vol + air_gap_vol_bottom, dest.top(z = -5), rate = reagent.flow_rate_dispense)
        #time.sleep(5)
        if wait_time != 0:
            ctx.delay(seconds=wait_time, msg='Waiting for 5 seconds.')
        if air_gap_vol_bottom !=0:
            pipet.dispense(air_gap_vol_top, dest.top(z = -5), rate = reagent.flow_rate_dispense)
        pipet.blow_out(dest.top(z = -2))
        #pipet.touch_tip(v_offset = -4)
        if air_gap_vol_bottom != 0:
            pipet.aspirate(air_gap_vol_bottom, dest.top(z = -5),rate = reagent.flow_rate_aspirate) #air gap
        pipet.blow_out()
...
###############################################################################
    ...
    #Transfer lysis
        ...
        ctx.comment('Step '+str(STEP)+': '+STEPS[STEP]['description'])
        ctx.comment('###############################################')
        lysis_transfer_vol=[180, 180, 180] # Three rounds of 180
        x_offset = 0
        rinse = True
        i = 0
        ...
        if not m300.hw_pipette['has_tip']:
            pick_up(m300)
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 1)
        m300.dispense(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 3)
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 1)
        m300.dispense(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 3)
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 1)
        m300.dispense(180, location = Lysis.reagent_reservoir[Lysis.col].bottom(1), rate = 3)
        ...
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col], rate = 1)
        m300.dispense(180, location = work_destinations[i].top(0), rate = 3)
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col], rate = 1)
        m300.dispense(180, location = work_destinations[i].top(0), rate = 3)
        m300.aspirate(180, location = Lysis.reagent_reservoir[Lysis.col], rate = 1)
        m300.dispense(180, location = work_destinations[i].top(0), rate = 3)
        ...
        m300.aspirate(180, location = work_destinations[i].bottom(1), rate = 1)
        m300.dispense(180, location = work_destinations[i].bottom(1), rate = 3)
        m300.aspirate(180, location = work_destinations[i].bottom(1), rate = 1)
        m300.dispense(180, location = work_destinations[i].bottom(1), rate = 3)
        m300.aspirate(180, location = work_destinations[i].bottom(1), rate = 1)
        m300.dispense(180, location = work_destinations[i].bottom(1), rate = 3)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in supernatant_vol:
                ...
                pickup_height = pickup_height, rinse = False, wait_time = 5)
    ...
    if STEPS[STEP]['Execute']==True:
        ...
        for i in range(num_cols):
            ...
            for transfer_vol in vhb_wash_vol:
                ...
                pickup_height = 1, rinse = rinse, wait_time = 5)
    ...
    if (tip_track['counts'][m300] > 500):
        ctx.pause('Replace ' + str(m300.max_volume) + 'µl tipracks before \
        resuming.')
        m300.reset_tipracks()
